...
if opts.Preprocess
    ...
    [coeff,score,~,~,explained,~] = pca(yin','NumComponents',opts.ToKeep);
    y = score(:,1:find(cumsum(explained)>opts.var2Keep,1,'first'))';
...
%     NumComponents = max(find(cumsum(sigmas)>opts.var2Keep,1,'first'),opts.ToKeep);
